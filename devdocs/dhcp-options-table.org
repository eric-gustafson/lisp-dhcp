...
#+BEGIN_SRC lisp :tangle yes :var ltable=dhcp-options :filename "../dhcp-options-table.lisp"
  (in-package :dhcp-server)
  (defparameter *dhcp-options-table* ltable)
  (defparameter *dhcp-options-table-id* (dict))
  (loop :for row :in *dhcp-options-table* :do
     (setf (gethash (car row) *dhcp-options-table*) row))
#+END_SRC
